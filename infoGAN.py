...
class Model():
    ...
    def train_one_epoch(self, real_images, z_sample, c_sample, sess, ratio = 2):
...
if __name__ == "__main__":
    ...
    num=0
    '''
    for filename in os.listdir(r".//infogan_samples//train//"):            
        for c in os.listdir(r".//infogan_samples//train//"+str(filename)):
            w=cv2.imread(r".//infogan_samples//train//"+str(filename)+"//"+str(c))
            w=cv2.resize(w,(64,64))
            X_all.append(w)
            num+=1
            print("%d pictures get!" % (num))
    '''
    T=101
    while T-1:
        T=T-1
        ...
        for c in os.listdir(r".//infogan_samples//train//"+str(filename)):
            ...
            w=cv2.resize(w,(64,64))
            data = np.array(w)
            data = data.mean(axis=-1).reshape((64,64))
            X_all.append((data-127.5)/127.5)
            num+=1
            print("%d pictures get!" % (num))
            count=count+1
            if count==400 :
    ...
    if not os.path.exists("infogan_res/"):
        os.mkdir("infogan_res/")
        ...
        if epoch>199:
            for index, Img in enumerate(img[:100]):
                ...
                if len(Img.shape) == 3:
                    ...
                    Image.fromarray(Img.astype(np.uint8)).save("infogan_res/" + str(index) + "_" + str(epoch) + ".png")
